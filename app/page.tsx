...
const competitors = [
  { name: "省多多", level: "A级", initials: "省", color: "#f97316", update: "官网发布美国跨境物流政策解读内容", insight: "以政策变化切入，强化北美卡车平台的服务场景。", channels: ["官网", "公众号", "小红书", "视频号", "抖音"], link: "http://sddltl.com/Default.aspx" },
  { name: "货马达", level: "A级", initials: "货", color: "#2563eb", update: "持续突出美国超大件与重型货物运输能力", insight: "传播重点集中在数字化查价、全美覆盖和特殊货物方案。", channels: ["官网", "公众号", "小红书", "视频号", "抖音"], link: "https://www.hmd-truck.com" },
];
...
export default function Home() {
  ...
  const visibleNews = useMemo(() => {
    ...
    return newsItems.filter((item) => (activeFilter === "全部" || item.category === activeFilter) && (!keyword || `${item.title}${item.summary}${item.tags.join("")}`.toLowerCase().includes(keyword)));
  }, [activeFilter, query]);
        ...
        <div className="workspace">
          <div className="main-column">
            <section className="morning-brief" id="today">
              ...
              <div className="brief-metrics"><div><span>24H 新增</span><strong>12</strong><small>条行业动态</small></div><div><span>市场温度</span><strong className="warm">偏热</strong><small>较昨日上升</small></div><div><span>风险事件</span><strong className="risk">03</strong><small>需销售关注</small></div><div><span>竞对更新</span><strong>02</strong><small>2 家 A 级竞对</small></div></div>
            </section>

            <section className="section-block" id="brief">
              <div className="section-heading"><div><span className="section-index">01</span><h2>今日热点 TOP 3</h2></div><span className="section-note">基于时效、行业影响与销售价值排序</span></div>
          ...
          <aside className="right-rail" id="competitors">
            <section className="rail-card competitor-card"><div className="rail-heading"><div><span className="live-dot" />竞对动态监控</div><button aria-label="更多竞对">•••</button></div><p className="rail-subtitle">2 家 A 级竞对 · 今日 2 条更新</p><div className="competitor-list">{competitors.map((company) => <article key={company.name}><div className="company-head"><span className="company-avatar" style={{ background: company.color }}>{company.initials}</span><div><strong>{company.name}</strong><span>{company.level}重点监控</span></div><a href={company.link} target="_blank" rel="noreferrer">官网 ↗</a></div><h3>{company.update}</h3><p>{company.insight}</p><div className="channel-row">{company.channels.map((channel) => <span key={channel}>{channel}</span>)}</div></article>)}</div><button className="outline-button">查看全部竞对动态 <span>→</span></button></section>
            ...
            <section className="rail-card source-card"><div className="rail-heading"><div>数据与版权说明</div></div><p>受版权限制的来源仅展示中文摘要与原文链接，不转载全文或图片。</p><div><span>主要来源</span><strong>FreightWaves</strong></div><div><span>历史保留</span><strong>365 天</strong></div><div><span>下次更新</span><strong>下周一 09:00</strong></div></section>
          </aside>
        </div>
      </section>
    </main>
